...
/**
 * Заслон для чужих условий продажи.
 *
 * Описания товаров изначально пришли от поставщика, и у части из них
 * в тексте остался его коммерческий хвост: «Рассрочка 0% до года. Лизинг.
 * Кредит. Безнал. Работаем с Юр.лицами». Скрипт импорта такие хвосты режет
 * (PLAN.md §5), но режет по границе перечисления характеристик — а если
 * хвост стоит в самом начале, резать не от чего, и он проходит целиком.
 *
 * Почему это не косметика. Текст уезжает разом в три места: в видимое
 * описание на карточке, в описание страницы для поиска и в разметку товара.
 * То есть сайт от своего имени обещает рассрочку и лизинг, которых мастер
 * не даёт, — человек звонит за ними и получает отказ, и дальше не верит
 * уже ничему, включая цены.
 *
 * Отбрасываем целиком, а не вырезаем предложения: разобрать чужой текст
 * на «наше» и «не наше» надёжно нельзя, а карточка без описания
 * предусмотрена — там встаёт описание, собранное из характеристик.
 *
 * Правится это в админке: поле `description` — наше, импорт его больше
 * не перезаписывает. Поэтому здесь не молчим, а называем товары в журнале
 * сборки: заслон нужен на случай недосмотра, а не вместо текста.
 */
const FOREIGN_TERMS =
  /рассрочк|лизинг|в\s*кредит|кредит\b|безнал|юр\.?\s*лиц|с\s*НДС|под ключ|купите/i;

const stripped: string[] = [];

function ownWords(text: string, slug: string): string {
  if (!text || !FOREIGN_TERMS.test(text)) return text;
  stripped.push(slug);
  return "";
}
...
const products: Product[] = rows.map((r) => ({
  ...
  image: r.image,
  specs: r.specs,
  description: ownWords(r.description, r.slug),
  tier: r.tier,
  featured: r.featured,
...
// Не молча: сборку не роняем — карточка без описания работает, — но
// заказчику эти товары нужно дописать своими словами, а увидеть это
// можно только здесь.
if (stripped.length) {
  console.warn(
    `\nУ ${stripped.length} товаров описание отброшено: в нём чужие условия ` +
      `продажи (рассрочка, лизинг, кредит). Допишите свой текст в админке:`
  );
  for (const slug of stripped) console.warn(`  · ${slug}`);
  console.warn("");
}
