...
export async function decodeImage(buffer) {
  ...
  if (buffer[0] === 0x89 && buffer[1] === 0x50) return decodePng(ab);
  if (buffer.subarray(0, 4).toString("ascii") === "RIFF") return decodeWebp(ab);
  if (buffer[0] === 0xff && buffer[1] === 0xd8) return decodeJpeg(ab);
 ...
 * в края тёмную кайму.
 */
export function downscale(image, width, height) {
 ...
 */
export async function writeVariants(image, { outDir, base, widths }) {
...
export const kb = (bytes) => `${(bytes / 1024).toFixed(0)} КБ`;
